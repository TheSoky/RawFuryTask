...
public class MazeGenerator : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Not for user editing but to keep value between edit and play mode")]
    private Cell[] centreCells = new Cell[4];
    public Cell[] CentreCells { get { return centreCells; } }
    ...
    public void MakeExit()
    {
        ...
        newCell.cScript.IsGoalCell = true;
    ...
    public void CompareWalls(Cell cCell, Cell nCell)
    {
        ...
        if (nCell.gridPos.x < cCell.gridPos.x)
        {
            ...
            cCell.cScript.NeighbourCells.Add(nCell.cScript);
            nCell.cScript.NeighbourCells.Add(cCell.cScript);
        }
    ...
    {
        if (wallID == 1) cScript.LeftWall.SetActive(false);
        else if (wallID == 2) cScript.RightWall.SetActive(false);
        else if (wallID == 3) cScript.UpWall.SetActive(false);
        else if (wallID == 4) cScript.DownWall.SetActive(false);
    }
    ...
    public void CreateCentre()
    {
        ...
        centreCells[0].cScript.NeighbourCells.Add(centreCells[1].cScript);
        centreCells[0].cScript.NeighbourCells.Add(centreCells[2].cScript);

        centreCells[1].cScript.NeighbourCells.Add(centreCells[0].cScript);
        centreCells[1].cScript.NeighbourCells.Add(centreCells[3].cScript);

        centreCells[2].cScript.NeighbourCells.Add(centreCells[0].cScript);
        centreCells[2].cScript.NeighbourCells.Add(centreCells[3].cScript);

        centreCells[3].cScript.NeighbourCells.Add(centreCells[2].cScript);
        centreCells[3].cScript.NeighbourCells.Add(centreCells[3].cScript);
    ...
    {
        if (mazeParent != null) DestroyImmediate(mazeParent);
    }
    ...
    public void InitValues()
    {
        ...
        cellSize = (cellPrefab.transform.localScale.x * 4) / cellPrefab.GetComponent<SpriteRenderer>().sprite.pixelsPerUnit;
